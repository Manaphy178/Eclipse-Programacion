...
public class Main {
	...
	static int menu() {
		...
		System.out.println(
				"21. Listado de canciones que el Titulo blah blah blah" + "\n22. Listado de artistas palindromos");
	...
	static void ejer21(ArrayList<Cancion> song) {
		double media = devolverMedia(song);
		for (Cancion cancion : song) {
			if (contarVocales(cancion.getTitulo()) < media && posicionImpar(cancion.getTitulo())) {
				cancion.mostrarDatos();
			}
		}
	}

	static int contarVocales(String titulo) {
		String vocales = "AEIOUaeiou";
		int cuantas = 0;
		for (int i = 0; i < titulo.length(); i++) {
			if (vocales.contains(String.valueOf(titulo.charAt(i)))) {
				cuantas++;
			}
		}
		return cuantas;
	...
	static double devolverMedia(ArrayList<Cancion> song) {
		String vocales = "AEIOUaeiou";
		int cuantas = 0;
		for (Cancion cancion : song) {
			for (int i = 0; i < cancion.getTitulo().length(); i++) {
				if (vocales.contains(String.valueOf(cancion.getTitulo().charAt(i)))) {
					cuantas++;
				}
			}
		}

		return cuantas / song.size();
	}

	static boolean posicionImpar(String titulo) {
		String vocales = "AEIOUaeiou";
		boolean si= false;
		for (int i = 0; i < titulo.length(); i++) {
			if (vocales.contains(String.valueOf(titulo.charAt(i)))) {
	...
	static void artistasPalin(ArrayList<Artista> arts) {
		for (Artista artista : arts) {
			if (isPalin(artista.getNombre())) {
