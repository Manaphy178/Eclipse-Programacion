...
<%
	...
	int cod_e =Integer.parseInt( request.getParameter("equipo"));
	Equipo equipo = bd.dameEquipo(cod_e);
					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								<form action="./operaciones.jsp?tipo=modificarequipo&modificar=<%=equipo.getCodE() %>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" name="nombre" id="nombre" placeholder="Nombre del equipo" style="width:450px" maxlength="50" value="<%=equipo.getNombre()%>"/>
										</div>
										
										<div class="">
											<select name="liga" id="liga">
											<%for(Liga l : bd.todasLigas()){ %>
												<option value="<%=l.getCod()%>" <%if (equipo.getCodL() == l.getCod()){%>selected<%} %>><%=l.getNombre() %></option>
												<%} %>
											</select>
