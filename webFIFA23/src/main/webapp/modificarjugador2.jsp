...
<%
	...
	int cod_j =Integer.parseInt( request.getParameter("jugador"));
	Jugador jugador = bd.dameJugador(cod_j);
					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								<form action="./operaciones.jsp?tipo=modificarjugador&modificar=<%=jugador.getCodJ() %>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" name="nombre" id="nombre" placeholder="Nombre del jugador" style="width:450px" maxlength="20" value="<%=jugador.getNombre() %>" />
										</div>
										<div class="">
											<select name="pierna" id="pierna" >
												<option value="Izquierda" <%if (jugador.getPierna().equalsIgnoreCase("izquierda")){ %>selected<% }%>>Izquierda</option>
												<option value="Derecha" <%if (jugador.getPierna().equalsIgnoreCase("derecha")){ %>selected<% }%>>Derecha</option>
											</select>
										...
										<%for (Equipo e: bd.todosEquipos()){ %>
											<option value="<%=e.getCodE()%>" <%if(jugador.getCodE() == e.getCodE()){ %>selected<%} %>><%=e.getNombre() %></option>
											<%} %>
										</select>
									
										</div>
										<div class="">
											<input type="text" name="pais" id="pais" placeholder="Pais del jugador" maxlength="20" value="<%=jugador.getPais() %>"/>
										</div>
										<div class="">
											<input type="number" name="altura" id="altura" placeholder="Altura del jugador" max="280" value="<%=jugador.getAltura()%>"/>
										</div>
