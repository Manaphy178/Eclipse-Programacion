...
public class metodos_basura {
	...
	public static void rellenarCanciones3(Cancion[] canciones) {
		canciones[0] = new Cancion("Amanecer en el Bosque", 3.2);
		canciones[1] = new Cancion("Cielo Estrellado", 4.5);
		canciones[2] = new Cancion("Ritmo del Alma", 2.8);
		canciones[3] = new Cancion("Susurros del Viento", 3.7);
		canciones[4] = new Cancion("Viaje a lo Desconocido", 5.1);
		canciones[5] = new Cancion("Melodía Serena", 2.5);
		canciones[6] = new Cancion("Horizontes Lejanos", 4.0);
		canciones[7] = new Cancion("Noches de Reflexión", 3.9);
		canciones[8] = new Cancion("Despertar del Corazón", 1.8);
		canciones[9] = new Cancion("Tiempo Eterno", 6.2);
		canciones[10] = new Cancion("Sueños Perdidos", 3.6);
		canciones[11] = new Cancion("Encuentro Mágico", 2.9);
	}
	...
	public static Disco[] rellenarDiscos(Cancion[][] canciones) {
		...
		String[] titulos = { "Luces en el Horizonte", "Melodía de Cristal", "Amanecer Eterno" };
		int[] aniosPublicacion = { 2005, 2010, 2013 };
		...
		String[] productoresMusicales = { "Luna Beat", "Sonic Pulse", "Aurora Harmony" };
		for (int i = 0; i < disco.length; i++) {
