...
public class ejer9_basura {
	...
	static void rellenarTodo(ArrayList<Aeropuerto> aero, ArrayList<Avion> avion, ArrayList<Pasajero> pasajero,
			ArrayList<Comandante> comand) {
		fillStringAero();
		fillStringAvion();
		fillStringPasajero();
		fillStringComandante();
		aero.add(new Aeropuerto(nomAeropuerto.get(0), ciuAeropuerto.get(0), codAeropuerto.get(0)));
		...
		matriAvion.remove(0);
		pasajero.add(
				new Pasajero(nomPasajero.get(0), apePasajero.get(0), numTelefono(), idPasajero.get(0), numEdadPas()));
		pasajero.get(0).setId(idPasajero.get(0));
		...
		comand.get(0).setId(idComandante.get(0));
